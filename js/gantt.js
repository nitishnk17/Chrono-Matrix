...
const GanttChart = (() => {
     const MAX_BARS = 5000;   // Canvas is fast enough for 5k bars at 60fps
     ...
     function sample(data, n) {
          ...
          const dl = data.filter(d => d.event === 'DEADLOCK_DETECTED');
          const rest = data.filter(d => d.event !== 'DEADLOCK_DETECTED');
          const step = Math.max(1, Math.floor(rest.length / (n - dl.length)));
          return [...dl, ...rest.filter((_, i) => i % step === 0)].sort((a, b) => a.ts - b.ts);
     }
     ...
     function init(data, tooltipEl) {
          allData = data;
          filteredData = data;
          displayData = sample(data, MAX_BARS);
          tooltip = tooltipEl;
          render();
          bindBus();
     }
     ...
     function render() {
          ...
          const rowH = Math.max(28, Math.floor(600 / threads.length));
          ...
          const timeExtent = d3.extent(displayData, d => d.ts);
          ...
          document.querySelectorAll('.ev-chk').forEach(chk => {
               // Default to all active style
               chk.style.opacity = '1.0';
               chk.addEventListener('click', (e) => {
                    const evType = chk.dataset.ev;
                    if (activeEventTypes.has(evType)) {
                         activeEventTypes.delete(evType);
                         chk.style.opacity = '0.35';
                    } else {
                         activeEventTypes.add(evType);
                         chk.style.opacity = '1.0';
                    }
                    render(); // Rebuild structure and drawing
               });
          });
     ...
     function onZoom(event) {
          ...
          // Cross-view updates throttled to 20fps
          const rescaled = currentTransform.rescaleX(xBase);
          const t0 = rescaled.invert(0), t1 = rescaled.invert(innerW);
          throttledEmitTimeRange({ t0, t1 });
          // Sync overview brush
          if (ovBrushGroup && xOv) {
     ...
     function updateFilter(scenario) {
          ...
          displayData = sample(filteredData, MAX_BARS);
          selectedThread = null;
          render();
     }
