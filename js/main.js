...
(async function () {
     ...
     const PAGE_META = {
          ...
          instructions: { title: 'Data Instructions', sub: 'How to generate, shape, and load Chrono-Matrix trace data', tab1: 'Instructions', tab2: 'Trace schema', tab3: 'Help' },
          aireport:   { title: 'AI Report',        sub: 'Automated trace overview, problem diagnosis, and optimization guidance', tab1: 'AI report',          tab2: 'Diagnosis',          tab3: 'Guidance' },
     ...
     window.__cmNavigate = navigateTo;

     function stopActive3dViews() {
          Timeline3D?.stop?.();
          GalaxyViz?.stop?.();
          FlowViz?.stop?.();
          active3dPage = null;
     ...
     const sidebarHomeTrigger = document.getElementById('sidebar-home-trigger');
     sidebarHomeTrigger?.addEventListener('click', showHomeScreen);
     sidebarHomeTrigger?.addEventListener('keydown', (event) => {
     ...
     async function processTraceData(raw) {
          ...
          tryInit('AIReport',    () => AIReport.init(data));
     ...
     document.getElementById('wc-open-instructions')?.addEventListener('click', () => {
          welcomeScreen?.classList.add('hidden');
          loading.classList.add('hidden');
