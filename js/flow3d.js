...
const FlowViz = (() => {
    ...
    function init(data) {
        ...
        const EXCLUDED = new Set(['THREAD_START', 'THREAD_END', 'THREAD_JOIN', 'COND_WAIT']);
        const filtered = data.filter(e => !EXCLUDED.has(e.event));
        const step = Math.max(1, Math.floor(filtered.length / 5000));
        streamEvents = filtered.filter((_, i) => i % step === 0).sort((a, b) => a.ts - b.ts);
        initialized = true;
    }
