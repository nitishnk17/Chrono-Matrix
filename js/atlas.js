...
const AtlasChart = (() => {
     ...
     const SPACE_MARGIN = { top: 10, right: 20, bottom: 54, left: 90 };
     ...
     function activeData() {
          const data = currentTimeRange
               ? filteredData.filter(d => d.ts >= currentTimeRange.t0 && d.ts <= currentTimeRange.t1)
               : filteredData;
     ...
     function renderTimeline() {
          ...
          data.forEach(d => {
               ...
               const x = xScl(d.ts);
               const w = Math.max(2, xScl(d.ts + Math.max(1, d.duration_us)) - x);
          ...
          canvas.addEventListener('mousemove', (e) => {
               ...
               tooltip.innerHTML = `
                    ...
                    <div class="tt-row"><span class="tt-key">Time</span><span class="tt-val">${formatUs(d.ts - te[0])}</span></div>
               ...
               tooltip.style.top = Math.max(e.clientY - 12, 10) + 'px';
          });
     ...
     function renderSpaceMap() {
          ...
          svg.append('g')
               ...
               .call(d3.axisBottom(xScl).ticks(6).tickFormat(i => formatUs((i / TIME_BINS) * tRange)));
          ...
          const legend = svg.append('g').attr('transform', `translate(${SPACE_MARGIN.left},${SPACE_MARGIN.top + innerH + 18})`);
          legend.append('text')
